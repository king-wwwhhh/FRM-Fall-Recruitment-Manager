...
async function fetchMails(seenUids, opts = {}) {
  ...
  try {
    ...
    // 增量策略：只按 UID 水位线粗筛（UID > lastUid 才算新），不叠加 SINCE 日期过滤。
    // [2026-09-15 修复] 旧代码用 { since, uid } 组合搜索，但 QQ 邮箱大量"高 UID 新邮件"
    // 的 Date 头早于 recentDays（QQ 投递保留原始发送日期），被 SINCE 静默丢弃，
    // 表现为"新邮件识别不了 / 收不到"。UID 水位线本身已能完整增量，since 纯属多余且有害，删除。
    const crit = {};
