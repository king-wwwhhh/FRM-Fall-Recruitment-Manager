...
rem 若压缩包已自带依赖（完整版），无需 Node.js，直接启动
if exist "%ELECTRON_EXE%" goto LAUNCH

rem ---------- 0) 需要首次安装依赖：检查 Node.js ----------
where node >nul 2>&1
if errorlevel 1 (
  %ERRBOX% "未检测到 Node.js。请先到 https://nodejs.org 下载安装 LTS 版本（安装时务必勾选 Add to PATH），装好后再双击本启动脚本。也可改用「完整版」压缩包，它内置依赖、无需安装。"
  pause
  exit /b 1
)
...
if not exist "%ELECTRON_EXE%" (
  %ERRBOX% "Electron 未安装成功（缺少 %ELECTRON_EXE%），多半是 Electron 二进制下载失败。请检查网络后重新双击本脚本。"
  pause
  exit /b 1
)
